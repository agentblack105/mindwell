...
function translateUI(lang) {
    ...
    if (currentUser) {
        if (welcomeUser) welcomeUser.textContent = `${t.welcomeBack} ${currentUser}`;
    ...
    if (typeof updateResourceCards === 'function') {
        updateResourceCards(t);
    }
    
    // Update assessment if active
    if (currentAssessment === 'phq9' && document.getElementById('phq9Assessment') && 
        !document.getElementById('phq9Assessment').classList.contains('hidden')) {
        if (typeof renderQuestion === 'function') renderQuestion('phq9');
    } else if (currentAssessment === 'gad7' && document.getElementById('gad7Assessment') && 
               !document.getElementById('gad7Assessment').classList.contains('hidden')) {
        if (typeof renderQuestion === 'function') renderQuestion('gad7');
    }
}